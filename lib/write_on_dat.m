...
function write_on_dat(case_settings, dat_path)
...
fprintf(fileID, ' $REFQ XCG=%s,', add_decimal(case_settings.x_cg));
if isfield(case_settings,'z_cg')
    fprintf(fileID, 'ZCG=%s,', add_decimal(case_settings.z_cg));
end
if isfield(case_settings,'sref')
    fprintf(fileID, 'SREF=%s,', add_decimal(case_settings.sref));
end
if isfield(case_settings,'lref')
    fprintf(fileID, 'LREF=%s,', add_decimal(case_settings.lref));
end
if isfield(case_settings,'latref')
    fprintf(fileID, 'LATREF=%s,', add_decimal(case_settings.latref));
end
if isfield(case_settings,'b_layer')
    fprintf(fileID, 'BLAYER=%s,\n ', case_settings.b_layer);
end
if isfield(case_settings,'roughness')
    fprintf(fileID, 'ROUGH=%s,', add_decimal(case_settings.roughness));
end
if isfield(case_settings,'rhr')
    fprintf(fileID, 'RHR=%s,', add_decimal(case_settings.rhr));
end
if isfield(case_settings,'scale')
    fprintf(fileID, 'SCALE=%s,', add_decimal(case_settings.scale));
end
fprintf(fileID, '$\n');
...
if length(case_settings.alphas) ~= 1
    fprintf(fileID, 'NALPHA=%s,', add_decimal(length(case_settings.alphas)));
end
fprintf(fileID, 'ALPHA=');
for i=1:length(case_settings.alphas)
    fprintf(fileID, '%s,', add_decimal(case_settings.alphas(i)));
end
fprintf(fileID, '\n ');
if isfield(case_settings,'beta')
    fprintf(fileID, 'BETA=%s,\n ', add_decimal(case_settings.beta));
end
if isfield(case_settings,'phi')
    fprintf(fileID, 'PHI=%s,', add_decimal(case_settings.phi));
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'mach')
    fprintf(fileID, 'NMACH=%s,', add_decimal(length(case_settings.mach)));
    fprintf(fileID, 'MACH=');
    for i=1:length(case_settings.mach)
        fprintf(fileID, '%s,', add_decimal(case_settings.mach(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'ren')
    ...
    for i=1:length(case_settings.ren)
        fprintf(fileID, '%s,', add_decimal(case_settings.ren(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'alt')
    ...
    for i=1:length(case_settings.alt)
        fprintf(fileID, '%s,', add_decimal(case_settings.alt(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'vinf')
    ...
    for i=1:length(case_settings.vinf)
        fprintf(fileID, '%s,', add_decimal(case_settings.vinf(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'tinf')
    ...
    for i=1:length(case_settings.tinf)
        fprintf(fileID, '%s,', add_decimal(case_settings.tinf(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'pinf')
    ...
    for i=1:length(case_settings.pinf)
        fprintf(fileID, '%s,', add_decimal(case_settings.pinf(i)));
    end
    fprintf(fileID, '\n ');
end
fprintf(fileID, '$END\n');
...
if isfield(case_settings,'x0')
    fprintf(fileID, 'X0=%s,\n ', add_decimal(case_settings.x0));
end
if isfield(case_settings,'nose_type')
    fprintf(fileID, 'TNOSE=%s,\n ', case_settings.nose_type);
end
if isfield(case_settings,'power')
    fprintf(fileID, 'POWER=%s,\n ', add_decimal(case_settings.power));
end
if isfield(case_settings,'nose_length')
    fprintf(fileID, 'LNOSE=%s,\n ', add_decimal(case_settings.nose_length));
end
if isfield(case_settings,'nose_diameter')
    fprintf(fileID, 'DNOSE=%s,\n ', add_decimal(case_settings.nose_diameter));
end
if isfield(case_settings,'nose_bluntness')
    fprintf(fileID, 'BNOSE=%s,\n ', add_decimal(case_settings.nose_bluntness));
end
if isfield(case_settings,'trunc')
    fprintf(fileID, 'TRUNC=%s,\n ', case_settings.trunc);
end
if isfield(case_settings,'ctrbody_length')
    fprintf(fileID, 'LCENTR=%s,\n ', add_decimal(case_settings.ctrbody_length));
end
if isfield(case_settings,'ctrbody_diameter')
    fprintf(fileID, 'DCENTR=%s,\n ', add_decimal(case_settings.ctrbody_diameter));
end
if isfield(case_settings,'aftbody_type')
    fprintf(fileID, 'TAFT=%s,\n ', case_settings.aftbody_type);
end
if isfield(case_settings,'aftbody_length')
    fprintf(fileID, 'LAFT=%s,\n ', add_decimal(case_settings.aftbody_length));
end
if isfield(case_settings,'aftbody_diameter')
    fprintf(fileID, 'DAFT=%s,\n ', add_decimal(case_settings.aftbody_diameter));
end
if isfield(case_settings,'nozzle_diameter')
    fprintf(fileID, 'DEXIT=%s,\n ', add_decimal(case_settings.nozzle_diameter));
end
fprintf(fileID, '$END\n');
...
if isfield(case_settings,'n_prot')
    fprintf(fileID, 'NPROT=%s,\n ', add_decimal(case_settings.n_prot));
end
...
if isfield(case_settings,'x_prot')
    ...
    for i = 1:case_settings.n_prot
        fprintf(fileID, '%s,', add_decimal(case_settings.x_prot(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'prot_in_set')
    ...
    for i = 1:length(case_settings.n_prot)
        fprintf(fileID, '%s,', add_decimal(case_settings.prot_in_set(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'l_prot')
    ...
    for i = 1:length(case_settings.l_prot)
        fprintf(fileID, '%s,', add_decimal(case_settings.l_prot(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'w_prot')
    ...
    for i = 1:length(case_settings.w_prot)
        fprintf(fileID, '%s,', add_decimal(case_settings.w_prot(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'h_prot')
    ...
    for i = 1:length(case_settings.h_prot)
        fprintf(fileID, '%s,', add_decimal(case_settings.h_prot(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'o_prot')
    ...
    for i = 1:length(case_settings.o_prot)
        fprintf(fileID, '%s,', add_decimal(case_settings.o_prot(i)));
    end
    fprintf(fileID, '\n ');
end
fprintf(fileID, '$END\n');
...
if isfield(case_settings,'semi_span')
    ...
    for i = 1:length(case_settings.semi_span)
        fprintf(fileID, '%s,', add_decimal(case_settings.semi_span(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'chord')
    ...
    for i = 1:length(case_settings.semi_span)
        fprintf(fileID, '%s,', add_decimal(case_settings.chord(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'x_le')
    ...
    for i = 1:length(case_settings.x_le)
        fprintf(fileID, '%s,', add_decimal(case_settings.x_le(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'sweep')
    fprintf(fileID, 'SWEEP=');
    for i = 1:length(case_settings.sweep)
        fprintf(fileID, '%s,', add_decimal(case_settings.sweep(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'ler')
    ...
    for i = 1:length(case_settings.ler)
        fprintf(fileID, '%s,', add_decimal(case_settings.ler(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'n_fins')
    fprintf(fileID, 'NPANEL=%s,\n ', add_decimal(case_settings.n_fins));
end
if isfield(case_settings,'z_upper')
    fprintf(fileID, 'ZUPPER=');
    for i = 1:length(case_settings.z_upper)
        fprintf(fileID, '%s,', add_decimal(case_settings.z_upper(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'z_lower')
    ...
    for i = 1:length(case_settings.z_lower)
        fprintf(fileID, '%s,', add_decimal(case_settings.z_lower(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'l_max_upper')
    ...
    for i = 1:length(case_settings.l_max_upper)
        fprintf(fileID, '%s,', add_decimal(case_settings.l_max_upper(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'l_max_lower')
    ...
    for i = 1:length(case_settings.l_max_lower)
        fprintf(fileID, '%s,', add_decimal(case_settings.l_max_lower(i)));
    end
    fprintf(fileID, '\n ');
end   
if isfield(case_settings,'l_flat_upper')
    ...
    for i = 1:length(case_settings.l_flat_upper)
        fprintf(fileID, '%s,', add_decimal(case_settings.l_flat_upper(i)));
    end
    fprintf(fileID, '\n ');
end
if isfield(case_settings,'l_flat_lower')
    ...
    for i = 1:length(case_settings.l_flat_lower)
        fprintf(fileID, '%s,', add_decimal(case_settings.l_flat_lower(i)));
    end
    fprintf(fileID, '\n ');
end   
if isfield(case_settings,'sta')
    fprintf(fileID, 'STA=');
    fprintf(fileID, '%s,', add_decimal(case_settings.sta));
    fprintf(fileID, '\n ');
end 
if isfield(case_settings,'phi_fins')
    fprintf(fileID, 'PHIF=');
    for i = 1:length(case_settings.phi_fins)
        fprintf(fileID, '%s,', add_decimal(case_settings.phi_fins(i)));
    end
    fprintf(fileID, '\n ');
end
fprintf(fileID, '$END\n');
