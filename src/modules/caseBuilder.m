...
% Unità di misura
datcom_case.unit = "M";  % M metri, CM centiemtri, IN inches, ft feet

% REFQ
datcom_case.x_cg = 1.29; % distanza CG longitudinale dalla sommità del nosecone
...
datcom_case.roughness = 0.002; % Altezza della rugosità, in centimetri se sistema metrico (anche metri), default 0
...
datcom_case.alt = 300; % altitudini (tante quanti sono i mach o gli altri dati), default 0
datcom_case.mach = [0.4]; % numeri di mach (almeno 1)
% datcom_case.ren = [3.E06]; % Reynolds (tanti quanti sono i mach o gli altri dati)
...
% datcom_case.power = 0.7; % se power definire il coefficiente
datcom_case.nose_length = 0.55; % lunghezza nose
datcom_case.nose_diameter = 0.1; % diametro
% datcom_case.nose_bluntness = 0.3 % raggio di troncamento
% datcom_case.trunc = 'TRUE'; % se troncato mettere TRUE, default FALSE
datcom_case.ctrbody_length = 0.140; % lunghezza del corpo centrale (fusoliera) dopo il nose
datcom_case.ctrbody_diameter = 0.1; % diametro del corpo centrale, default nose_diameter
...
datcom_case.aftbody_diameter = 0.0698; % diametro dell'afterbody, diverso da 0 e ctrbody_diameter
datcom_case.nozzle_diameter = 0.05; % diametro ugello per calcolo del drag della base, default restituisce 0 base drag 
...
datcom_case.semi_span = [0.0, 0.11]; % distanza semi-span, 0.0 se le alette sono direttamente sulla fusoliera
datcom_case.chord = [0.24, 0.12]; % corda del singolo pannello ad ogni semi-span definito
datcom_case.x_le = [1.71, 1.77]; % distanza dalla punta del nose per ogni singolo semi-span definito
...
datcom_case.z_upper = [0.00625, 0.0125]; % rapporto spessore su corda della parte superiore della sezione HEX o ARC
% datcom_case.z_lower = []; % rapporto spessore su corda della parte inferiore della sezione HEX o ARC, default zupper
datcom_case.l_max_upper = [0.25, 0.25]; % Frazione della corda dalla sezione del bordo d'attacco allo spessore massimo della superficie superiore. 
...
datcom_case.l_flat_upper = [0.5, 0.5]; % La frazione della corda di spessore costante, sezione superiore
